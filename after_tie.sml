...
structure Show = struct
   ...
   (* user-defined types *)
   fun inj inj (IN b) = IN (b o Pair.map (Fn.id, inj))
   ...
   in
      ...
      (* datatypes *)
      fun (IN l) + (IN r) = IN (fn (rs, Sum.INL a) => l (rs, a)
                                 | (rs, Sum.INR b) => r (rs, b))
